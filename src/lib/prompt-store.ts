...
const CONFIG_PATH = 'prompts/config.json';
...
export async function writeConfig(next: Partial<Config>): Promise<Config> {
  ...
  await writeFile(resolve(CONFIG_PATH), JSON.stringify(merged, null, 2) + '\n', 'utf8');
  return merged;
}
